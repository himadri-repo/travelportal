 ...
 *
 */
if(!class_exists('STPluginsImporter'))
{
    class STPluginsImporter
    {
        static $_dir;
        static $_url;
        static $_import_path;
        static $_package='default';
        static $_import_page='admin.php?page=stp_importer';
        static $_import_url='';
        ...
        {
            if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'stp_importer'){
                wp_enqueue_script('stp_importer_js',self::$_url.'/js/import.js');
                wp_enqueue_style('stp_importer_css',self::$_url.'/css/custom.css');
                wp_enqueue_script('bootstrap-js',self::$_url.'/js/bootstrap.js');
                wp_enqueue_style('bootstrap-css',self::$_url.'/css/bootstrap.css');
                wp_localize_script('jquery','stp_importer',array(
                    'admin_url'=>admin_url(),
                    'loading_src'=> admin_url( '/images/spinner.gif' )
                ));
            }
        }
        static function _add_sub_menu()
        {
            ...
            add_submenu_page(apply_filters( 'ot_theme_options_menu_slug', 'ot-theme-options' ),__('One-click Install Demo Content',STP_TEXTDOMAIN),__('One-click Install',STP_TEXTDOMAIN),'manage_options','stp_importer',array(__CLASS__,'_show_import_page'));
