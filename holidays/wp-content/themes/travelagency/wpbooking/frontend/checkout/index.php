...
if (empty($cart)) {
    wpbooking_set_message(esc_html__('Sorry! Your cart is currently empty', 'travelagency'), 'danger');
}
                ...
                <div class="row">
                    <div class="col-md-7">
                        <div class="checkout-form-wrap">
                            <h5 class="checkout-form-title"><?php esc_html_e('Billing Information', 'travelagency') ?></h5>
                            <h5 class="checkout-form-sub-title">
                                <?php
                                if(is_user_logged_in()){
                                    esc_html_e('Billing Information', 'travelagency');
                                }else{
                                    esc_html_e("Already have an account?","travelagency");
                                    $url_check_out = get_permalink(wpbooking_get_option('checkout_page'));
                                    ?>
                                    <a href="<?php echo esc_url(add_query_arg(array("redirect_to"=>$url_check_out),wp_login_url())); ?>" class="text-color"><?php esc_html_e('Login',"travelagency") ?></a>
                                <?php } ?>
                            </h5>
                    ...
                    <div class="col-md-5">
                        <div class="wpbooking-checkout-review-order <?php if($service_type == 'tour'){ echo esc_attr('tour'); } ?>">
                            ...
                            <h5 class="checkout-form-title"><?php esc_html_e('Method of Payment', 'travelagency') ?></h5>
                            ...
                            <div class="form-group">
                                <label for="term_condition">
                                    ...
                                    $link = '<a target="_blank" class="term_condition" href="'.esc_url(get_permalink($page_term_condition)).'">'.esc_attr__("terms and conditions", 'travelagency').'</a>';
                                    printf(esc_html__("I have read and accepted the %s","travelagency"),$link);
                                    ?>
                                </label>
                            </div>
                            <div class="checkout-submit-button">
                                <button type="submit" class="wb-btn wb-btn-primary wb-btn-md submit-button" disabled><?php esc_html_e('CHECKOUT', 'travelagency') ?></button>
                            </div>
                        </div>
                    </div>
                </div>
