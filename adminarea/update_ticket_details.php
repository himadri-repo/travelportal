...
{
	$sql_update="UPDATE tickets_tbl SET pnr='".$_POST["pnr"]."' WHERE id='".$_POST["ticket_id"]."'";
	
	if(mysql_query($sql_update))
	{
		$sql="SELECT t.no_of_person,u.email FROM tickets_tbl INNER JOIN user_tbl u ON t.user_id=u.id WHERE t.id='".$_POST["ticket_id"]."'";
		$result=mysql_query($sql);
		$row=mysql_fetch_array($result);
		$email=$row["email"];
		$link="";
				 $msg="Pnr Added to your Ticket by Admin";
				 $msg1="";
				 $msg2="";
			     $subject="PNR Update";
		...
		$msg="<div class='alert alert-success'>PNR Updated Successfully</div>";
	}
	else
	{
					    ...
					    <div class="row">
						   <div class="form-group has-info col-sm-6" style="float:left;margin-top:15px">									
							<button type="submit" class="pull-left btn btn-sm btn-primary" name="update">															
								<span class="bigger-110">Update PNR</span>
							</button>																		
							 </div>
						</div> 
					    </form>
