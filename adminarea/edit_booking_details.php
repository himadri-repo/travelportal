									...
									<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-bottom:20px">
										...
										<tbody id="grid">											                                            
                                              <?php
												  ...
												  $sql_check="SELECT * FROM refrence_booking_tbl WHERE refrence_id='".$_GET["id"]."'";
												  $result_check=mysql_query($sql_check);
												  if(mysql_num_rows($result_check)==0)
												  {
												  ...
												  {
													  $sql_details="SELECT * FROM refrence_booking_tbl WHERE refrence_id='".$_GET["id"]."' ORDER BY id ASC";
												  }
                                            ...
                                            <div class="form-group has-info col-xs-12 col-sm-12">
													...
													<div class="col-xs-3">
														...
														<select id="approved"  name="approved" class="col-xs-12 col-sm-12" required>
														   ...
														   <?php }  if($row["status"]=="REQUESTED FOR CANCEL") {?>
							...
							<?php while($sellerrows = mysql_fetch_array($seller_result, MYSQL_ASSOC)) {?>
										<tbody id="grid">
											  <tr style="background:#fff">
												...
												$qty = $qty - $orderqty;
												?>
												<td><input type="number" name="no_of_person" class="col-xs-12 col-sm-12" value="<?php echo $orderqty?>" style="width:50px"></td>
											  </tr>                                             											  											                                                                                  
										</tbody>
							<?php }?>
						<?php }
										...
										<tbody id="grid">
											  <tr style="background:#fff">
												<td><span>No sellers found</span></td> 
											  </tr>
										</tbody>
							<?php } ?>
									</table>							
