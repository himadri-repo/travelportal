...
{   		 
    if(localStorage.getItem("faq_id")==null)
    {
	...
	$("#txt_title").bind("keyup",function(e)
	{
	  ...
	  else
	  {
		...
		$("#txt_title").parent().append('<div id="div_msg" style="color:#F87279" class="alert alert-block alert-danger col-xs-12 col-sm-12 ">FAQ can not be left blank !!!</div>');
	  }
	});
  ...
  else
  {
     ...
     $.ajax
			({
				...
				type: "POST",      	
				data: "question="+$("#txt_title").val()+"&description="+CKEDITOR.instances.editor.getData()+"&tag=update&faq_id="+localStorage.getItem("faq_id"),      		
				cache: false,					  			
...
function get_record()
{
   ...
   $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/faq_sql.php',   	
				type: "POST",      	
				data: "tag=edit"+"&faq_id="+localStorage.getItem("faq_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
