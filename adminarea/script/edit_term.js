...
{   		 
    ...
    {
	  window.location.href=''+baseurl+'/adminarea/term.php';
	}
  ...
  else
  {
     ...
     $.ajax
			({
				...
				type: "POST",      	
				data: "title="+$("#txt_title").val()+"&description="+CKEDITOR.instances.editor.getData()+"&tag=update&term_id="+localStorage.getItem("term_id"),      		
				cache: false,					  			
...
function get_record()
{
   ...
   $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/term_sql.php',   	
				type: "POST",      	
				data: "tag=edit"+"&term_id="+localStorage.getItem("term_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
