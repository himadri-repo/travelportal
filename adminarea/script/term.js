...
function get_all(start)
{
	...
	$("#div_pagination").html('');
    $.ajax({
				url: ''+baseurl+'/adminarea/ajax/term_sql.php',   	
				type: "POST",      	
...
function del(x)
{
 ...
 localStorage.setItem("term_id",x);
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/term_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&term_id="+localStorage.getItem("term_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("term_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_term.php';
