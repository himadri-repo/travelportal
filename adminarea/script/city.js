					...
					$.each(data, function(k,v) 
					{
					   if(k=='no_records')
							   ...
							   {    
                                    ...
                                    $('#grid_row'+k+"").append("<td>"+data[k]['city']+"</td>");																																														
									$('#grid_row'+k+"").append("<td>"+data[k]['code']+"</td>");
...
function del(x)
{
 ...
 localStorage.setItem("city_id",x);
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/city_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&city_id="+localStorage.getItem("city_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("city_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_city.php';
