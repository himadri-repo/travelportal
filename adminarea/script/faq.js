...
{   
    localStorage.removeItem("faq_id");    
    $("#faq").addClass('active');
					...
					$.each(data, function(k,v) 
					{
					   if(k=='no_records')
							   ...
							   {    
									...
									$('#grid_row'+k+"").append("<td>"+data[k]['question']+"</td>");
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/faq_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&faq_id="+localStorage.getItem("faq_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("faq_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_faq.php';
