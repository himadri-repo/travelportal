...
function get_all(start)
{
    ...
    $.ajax({
				...
				type: "POST",      	
				data: "tag=search"+"&start="+start+"&field="+$("#field").val()+"&value="+value+"&dt_from="+$("#dt_date_from").val()+"&dt_to="+$("#dt_date_to").val()+"&allowEmptyStock="+allowEmptyStock+"&sector="+sector,
				cache: false,							
					...
					$.each(data, function(k,v) 
					{
					   if(k=='no_records')
							   ...
							   {    
								    ...
								    if(data[k]['sale_type']=="quote")
								    	$('#grid_row'+k+"").append("<td></td>");
								    else
								      $('#grid_row'+k+"").append("<td>"+data[k]['no_of_person']+"</td>");
									
									$('#grid_row'+k+"").append("<td>"+data[k]['name']+" <br/>( "+data[k]['uid']+" ) "+"</td>");
									if(data[k]['no_of_person']<=0)
									{
									  $('#grid_row'+k+"").append("<td>NO</td>");
									}
									else
									{
									    $('#grid_row'+k+"").append("<td>"+data[k]['available']+"</td>");
									}
									...
									$('#grid_row'+k+"").append("<td>"+
									 ...
									 "<div class='hidden-sm hidden-xs action-buttons'>"+														
										...
										"<a class='red' style='cursor:pointer'  href='edit_ticket_details.php?id="+data[k]['id']+"'>"+
