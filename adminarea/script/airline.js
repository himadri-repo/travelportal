...
function get_all(start)
{
	...
	from=(start*10)+1;
	$("#grid").html('');
	$("#grid").html('<tr ><td colspan="7" align="center"><img   src="'+baseurl+'assets/images/loading.gif" alt="Uploading...."/></td></tr>');
	$("#div_pagination").html('');
    $.ajax({
					...
					$.each(data, function(k,v) 
					{
					   if(k=='no_records')
							   ...
							   {    
                                    ...
                                    $('#grid_row'+k+"").append("<td>"+data[k]['airline']+"</td>");																																														
									$('#grid_row'+k+"").append("<td>"+data[k]['aircode']+"</td>");
...
function del(x)
{
 ...
 localStorage.setItem("airline_id",x);
 var box=bootbox.dialog(
		{
			message:'Do You Want to Delete This Airline ???',
			title: "Confrim",
			buttons:
			{
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/airline_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&airline_id="+localStorage.getItem("airline_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("airline_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_airline.php';
