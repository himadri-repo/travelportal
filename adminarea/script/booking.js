...
function get_all(start)
{
	...
	$("#div_pagination").html('');
    $.ajax({
				url: ''+baseurl+'/adminarea/ajax/confirm_booking_sql.php',   	
				type: "POST",      	
					   ...
					   if(k=='no_records')
							   {
                                    $("#info").html(''); 							   
									$("#grid").append("<tr></tr>"); 
						            $("#grid").append("<td colspan='11' align='center' style='color:red' > "+v+" </td>");									
							   }
							   else if(k=='records')
							   {							   
							    if(v>1)
							   ...
							   {    
									...
									$('#grid_row'+k+"").append("<td>"+data[k]['booking_id']+"</td>");
									...
									if(data[k]['user_id']!=data[k]['seller_id'])
									{
										$('#grid_row'+k+"").append("<td>"+									
										 "<div class='hidden-sm hidden-xs action-buttons'>"+														
											...
											"<a class='red' style='cursor:pointer'  href='edit_confirm_booking_details.php?id="+data[k]['id']+"'>"+
