...
{   
    localStorage.removeItem("profession_id");    
...
function del(x)
{
 ...
 localStorage.setItem("profession_id",x);
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/profession_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&profession_id="+localStorage.getItem("profession_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("profession_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_profession.php';
