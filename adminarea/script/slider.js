...
{   
    localStorage.removeItem("slider_id");    
	...
	$("#all_posts_menu").addClass('active');
	 ...
	 $("#btn_add").click(function(){
	     window.location.href=''+baseurl+'/adminarea/add_slider.php';
	 });
	get_all(0);				
});     
...
function get_all(start)
{
	...
	from=(start*10)+1;
	$("#grid").html('');
	$("#grid").html('<tr ><td colspan="7" align="center"><img   src="'+baseurl+'/adminassets/images/loading.gif" alt="Uploading...."/></td></tr>');
	$("#div_pagination").html('');
    $.ajax({
					   ...
					   if(k=='no_records')
							   {
                                    $("#info").html(''); 							   
									$("#grid").append("<tr></tr>"); 
						            $("#grid").append("<td colspan='7' align='center' style='color:red' > No Record To Display </td>");									
							   }
							   else if(k=='records')
							   {
							   ...
							   {    
									...
									$('#grid_row'+k+"").append("<td>"+data[k]['slider_id']+"</td>");
									$('#grid_row'+k+"").append("<td>"+data[k]['slider_title']+"</td>");
									$('#grid_row'+k+"").append("<td>"+data[k]['slider_sub_title']+"</td>");
									$('#grid_row'+k+"").append("<td>"+data[k]['slider_description']+"</td>");								
									$('#grid_row'+k+"").append("<td>"+
															 "<div class='hidden-sm hidden-xs action-buttons'>"+														
																"<a class='green' style='cursor:pointer' onclick='edit("+data[k]['slider_id']+")'>"+
																	"<i class='ace-icon fa fa-pencil bigger-130'></i>"+
																"</a>"+
																"<a class='red' style='cursor:pointer' onclick='del("+data[k]['slider_id']+")'>"+
...
function del(x)
{
 ...
 localStorage.setItem("slider_id",x);
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/slider_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&slider_id="+localStorage.getItem("slider_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("slider_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_slider.php';
