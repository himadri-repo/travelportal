...
function del(x)
{
 ...
 localStorage.setItem("testimonial_id",x);
...
function del_record()
{
  $.ajax
			({
				url: ''+baseurl+'/adminarea/ajax/testimonial_sql.php',   	
				type: "POST",      	
				data: "tag=delete"+"&testimonial_id="+localStorage.getItem("testimonial_id"),      		
				cache: false,					  			
				success: function(data)  		
				{
					var data	=	$.parseJSON(data);	
...
function edit(x)
{
    ...
    localStorage.setItem("testimonial_id",x);
	window.location.href=''+baseurl+'/adminarea/edit_testimonial.php';
