...
$birth_region=isset($_POST['birth_region'])? $_POST['birth_region']:"";
$profession=isset($_POST['hid_profession'])? $_POST['hid_profession']:"";
$nationality=isset($_POST['nationality'])? $_POST['nationality']:"";
$dob=isset($_POST['dob'])? $_POST['dob']:"";
$dod=isset($_POST['dod'])? $_POST['dod']:"";
$zodiac_sign=isset($_POST['zodiac_sign'])? $_POST['zodiac_sign']:"";
$image=isset($_POST['hid_image'])? $_POST['hid_image']:"";
...
if(isset($_POST["btn_create"]))
{
	...
	$arr=explode("/",$dod);
	$dod=$arr[2]."-".$arr[1]."-".$arr[0];
	$count=0;
	 foreach ($_FILES['file_image']['name'] as $filename) 
	{
		$temp="../upload/";
		$tmp=$_FILES['file_image']['tmp_name'][$count];
		$temporary = explode(".", $_FILES["file_image"]["name"][$count]);		
		$file_extension = end($temporary);
		$count=$count + 1;
		$file="celebirty_".time().mt_rand(10,100).".".$file_extension;
		$temp=$temp.$file;
		if(move_uploaded_file($tmp,$temp))
		{
			$filepath = "../upload/".$file;
			require_once('ajax/php_image_magician.php');
			$magicianObj = new imageLib($filepath);
			$magicianObj->resizeImage(255,170,2);		
			$success=$magicianObj->saveImage("../upload/thumb/".$file,100);
		}
    ...
    $main_profession=explode(",",$profession);
    $main_profession=$main_profession[0];	
	$sql="INSERT INTO celebrity_tbl(id,name,profession,dob,dod,description,zodiac_sign,image,birth_region,birth_place,priority,popularity,main_profession,nationality) VALUES
	('','$name','$profession','$dob','$dod','$description','$zodiac_sign','$image','$birth_region','$birth_place','0','0','$main_profession','$nationality')";
	$result=mysql_query($sql) or die(mysql_error());
	if($result)
	{
		$_SESSION["cel_msg"]="Celebrity Added Successfully";
		?>
...
if(isset($_POST["btn_update"]))
{
	...
	$sql="INSERT INTO wallet_tbl (user_id,date,amount,type)values('".$_GET["id"]."','".date("Y-m-d h:i:s")."','".$_POST["amount"]."','CR')";
	$result=mysql_query($sql);

   
	if($result)
	{
		$_SESSION["cel_msg"]="Money Added Successfully";
		?>
		<script>window.location.href="celebrity.php"</script>
		<?php
	}
							    ...
							    <div class="widget-box" style="float:left;width:100%"><!--Widget Box start-->
									...
									<div class="widget-body"><!--Widget Body start-->																											
											<div class="form-group has-info col-xs-12 col-sm-6 pink">
													<label class="col-sm-12 control-label" for="form-field-1" style="text-align:left"> Money (required) </label>
													<div class="col-xs-12 col-sm-12">
														<input type="text" id="amount" name="amount" placeholder="Money" class="col-xs-12 col-sm-12" value="" required>														
													</div>
											</div>
