    ...
    <div class="container-base">
        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-lg-12 col-md-12">
                ...
                $title = $outbound['source_city'].' - '.$outbound['destination_city'];
                $dep_date_time = date('D - d-M-Y', strtotime($outbound['departure_date_time']));
                $dep_time = date('H:i', strtotime($outbound['departure_date_time']));
                $arv_date_time = date('D - d-M-Y', strtotime($outbound['arrival_date_time']));
                $arv_time = date('H:i', strtotime($outbound['arrival_date_time']));

                $dateDiff = intval((strtotime($outbound["arrival_date_time"])-strtotime($outbound["departure_date_time"]))/60);

                $airline_name = $outbound['airline_name'];
                $flight_no = $outbound['flight_no'];
                $refundable = ($outbound['refundable'] === 'Y') ? 'Refundable' : 'Not Refundable';
                
                $ob_fare_quote = $outbound['api_fare_quote'];
                $ib_fare_quote = $inbound['api_fare_quote'];
                ?>

                <!-- Start : Flight Itinerary details -->
                <div id="dvflightdetails" class="flight-section left">
                    <!-- Start of outbound trip -->
                    <div class="section-head flight">
                        <span>Flight Detail - Outbound</span>
                        <?php if(boolval($ob_fare_quote['isprice_changed'])) { ?>
                        <div class="tv_right">
                    ...
                    <div class="itinerary">
                        <div class="departure">
                            ...
                            <div class="flight-item">
                                ...
                                <!-- Flight details -->
                                <?php if($ob_fare_quote && $ob_fare_quote['ob_flight_segments'] && is_array($ob_fare_quote['ob_flight_segments']) && count($ob_fare_quote['ob_flight_segments']) > 0) {
                                    $index = 0;
                                    foreach ($ob_fare_quote['ob_flight_segments'] as $quote) { 
                                        $index++;
                                        ...
                                        <div class="flgith-details">
                                            <div class="fli1">
                                                <div class="fli1-m">
                                                    <div class="fli1-m-l"><img alt="Flight" width="50" height="35" src="/upload/<?= $outbound['image']?>"></div>
                                                    ...
                                                    <div class="bageg-tab-b2b-new">
                                                        <div class="bagg-b2b-air">
                                                            <div class="fli-lo-new-bagge">
                                                                <span class="logo-xs ai-lg"><img width="28" height="28" src="/upload/<?= $inbound['image']?>"></span>
                                                            </div>
                ...
                <div id="dvcommercial" class="commercial-section right stickyitem">
                    ...
                    <div>
                        <?php if(($ob_fare_quote && $ob_fare_quote['passengers_fare'] && is_array($ob_fare_quote['passengers_fare']) && count($ob_fare_quote['passengers_fare']) > 0) &&
                            ...
                            $tax = round(floatval($ob_fare_quote['fare']['tax']) + floatval($ob_fare_quote['fare']['othercharges']) + floatval($ib_fare_quote['fare']['tax']) + floatval($ib_fare_quote['fare']['othercharges']) + $tds + (($total_spl_margin + $total_whl_margin) * ($adult + $child)) - $commision - $discount, 0);
                ...
                <?php
                    $title = $inbound['source_city'].' - '.$inbound['destination_city'];
                    $dep_date_time = date('D - d-M-Y', strtotime($inbound['departure_date_time']));
                    $dep_time = date('H:i', strtotime($inbound['departure_date_time']));
                    $arv_date_time = date('D - d-M-Y', strtotime($inbound['arrival_date_time']));
                    $arv_time = date('H:i', strtotime($inbound['arrival_date_time']));

                    $dateDiff = intval((strtotime($inbound["arrival_date_time"])-strtotime($inbound["departure_date_time"]))/60);

                    $airline_name = $inbound['airline_name'];
                    $flight_no = $inbound['flight_no'];
                    $refundable = ($inbound['refundable'] === 'Y') ? 'Refundable' : 'Not Refundable';
                    
                    $ib_fare_quote = $inbound['api_fare_quote'];
                ?>
                <!-- Start of inbound trip -->
                <div id="dvflightdetails_rtn" class="flight-section left">
                    <div class="section-head flight">
                        <span>Flight Detail - Inbound</span>
                        <?php if(boolval($ib_fare_quote['isprice_changed'])) { ?>
                        <div class="tv_right">
                            <div class="bg_si_d alert_msg blinking">Flight rate has been changed, due to heavy demand</div>
                        </div>
                        <?php } ?>
                    ...
                    <div class="itinerary">
                        <div class="departure">
                            ...
                            <div class="flight-item">
                                ...
                                <!-- Flight details -->
                                <?php if($ib_fare_quote && $ib_fare_quote['ob_flight_segments'] && is_array($ib_fare_quote['ob_flight_segments']) && count($ib_fare_quote['ob_flight_segments']) > 0) {
                                    $index = 0;
                                    foreach ($ib_fare_quote['ob_flight_segments'] as $quote) { 
                                        $index++;
                                        $qt_dep_time = date('H:i', strtotime($quote['DepTime']));
                                        $qt_arv_time = date('H:i', strtotime($quote['ArrTime']));
                                        $qt_dep_date_time = date('D - d-M-Y', strtotime($quote['DepTime']));
                                        ...
                                        <div class="flgith-details">
                                            <div class="fli1">
                                                <div class="fli1-m">
                                                    <div class="fli1-m-l"><img alt="Flight" width="50" height="35" src="/upload/<?= $inbound['image']?>"></div>
                                                    <div class="fli1-m-r"><span ><?= $airline_name?></span><span><?= $flight_no ?></span></div>
                                                </div>
                                            </div>
                                            <div class="fli-d-r">
                ...
                <div id="dvpayment" class="flight-section left">
                    ...
                    <form method="POST" id="bookticket" action="<?php echo base_url(); ?>search/pre_booking_round/<?= $outbound["id"] ?>/<?= $inbound["id"] ?>">
