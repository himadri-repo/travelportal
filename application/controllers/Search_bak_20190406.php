...
class Search extends Mail_Controller 
{
		...
		{	
		   ...
		   if($result["flight"][0]["no_of_person"]>0 && $result["flight"][0]["approved"]==1)
		   {
				  ...
				  if($result["flight"][0]["user_id"]==$this->session->userdata('user_id'))
					...
					$result["flight"][0]["price"]=floatval( $result["flight"][0]["price"]-$result["flight"][0]["discount"]+ $result["flight"][0]["markup"]);
				  else
				    ...
				    $result["flight"][0]["price"]=floatval( $result["flight"][0]["price"]-$result["flight"][0]["discount"]+ $result["flight"][0]["markup"] + $result["flight"][0]["admin_markup"]);
	...
	public function update_booking()
	{
		 ...
		 {	
				 ...
				 foreach($_REQUEST["prefix"] as $key=>$value)
				 {
				        $pnr=isset($_REQUEST["pnr"][$key])?$_REQUEST["pnr"][$key]:"";
					    $arr=array("prefix"=>$_REQUEST["prefix"][$key],
								 "first_name"=>$_REQUEST["first_name"][$key],
								 "last_name"=>$_REQUEST["last_name"][$key],
								 "mobile_no"=>$_REQUEST["mobile_no"][$key],
						...
						$result = $this->User_Model->update_table("customer_information_tbl",$arr,"id",$_REQUEST["hid_booking_id"][$key]);
				 }
				 //echo $this->db->last_query();die();
				 if($result==true)
				 {
