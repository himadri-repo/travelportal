...
Class Search_Model extends CI_Model
{
	...
	{            		  
		$this->db->select('t.id,t.source,t.destination,t.ticket_no,t.pnr,t.departure_date_time,t.arrival_date_time,t.total,t.price,t.admin_markup,t.markup,t.sale_type,t.refundable,c.city as source_city,ct.city as destination_city,t.no_of_person,t.class,a.image,t.user_id,t.no_of_stops');
	...
	{		
		...
		$this->db->distinct(); //Code changed to have distinct dates, where multiple tickets present in same date in different time slots
		$this->db->select("DATE_FORMAT(departure_date_time, '%d-%m-%Y') as departure_date_time");
		$this->db->from('tickets_tbl');	
		$this->db->order_by('departure_date_time', 'asc');
		$this->db->where($arr);
	...
	{		
		...
		$this->db->distinct(); //Code changed to have distinct dates, where multiple tickets present in same date in different time slots
		$this->db->select("DATE_FORMAT(departure_date_time, '%d-%m-%Y') as departure_date_time1");
		$this->db->from('tickets_tbl');
		$this->db->order_by('departure_date_time1', 'asc');
		$this->db->where($arr);
